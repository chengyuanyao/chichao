...
(function () {
  ...
  var PORTRAIT_PAINTERS = {
    ...
    mstorm: function (c) {
      ...
      pPoly(c, [[22, 62], [74, 62], [67, 53], [29, 53]], P_STORM);
      pPoly(c, [[74, 62], [58, 62], [56, 53], [67, 53]], 'rgba(10,10,6,.28)');
      c.strokeStyle = P_GOLDTRIM; c.lineWidth = 2.2;
      c.beginPath(); c.ellipse(48, 54, 20, 4.0, 0, 0, Math.PI * 2); c.stroke();
      pPoly(c, [[39, 53], [57, 53], [53, 16], [43, 16]], P_STORM);
      pPoly(c, [[57, 53], [52, 53], [50, 16], [53, 16]], P_STORM_L);
      pLine(c, 41, 51, 45, 17, 1.8, P_GOLDTRIM);
      pLine(c, 55, 51, 51, 17, 1.8, P_GOLDTRIM);
      c.strokeStyle = P_RUNE; c.lineWidth = 2.2;
      c.beginPath(); c.ellipse(48, 44, 16, 3.4, 0, 0, Math.PI * 2); c.stroke();
      c.beginPath(); c.ellipse(48, 32, 13, 2.8, 0, 0, Math.PI * 2); c.stroke();
      c.beginPath(); c.ellipse(48, 22, 10, 2.2, 0, 0, Math.PI * 2); c.stroke();
      pCirc(c, 38, 43, 2.0, P_FROST);
      pCirc(c, 58, 33, 1.8, P_RUNE);
      pCirc(c, 40, 23, 1.6, P_FROST);
      pPoly(c, [[42, 16], [54, 16], [48, 3]], P_FROST);
      pCirc(c, 48, 13, 3.2, P_RUNE);
      pCirc(c, 48, 6, 1.8, '#f4ffff');
      pPoly(c, [[32, 30], [42, 25], [40, 32], [48, 27], [42, 36], [34, 33]], P_RUNE);
      pLine(c, 56, 20, 72, 8, 2.0, '#e8ffff');
      pLine(c, 72, 8, 64, 16, 1.6, P_FROST);
    },
    /* ---- 秘法会（魔法阵营）单位 ---- */
    mage: function (c) {
      pShadow(c, 48, 60, 20);
    ...
    spider: function (c) {
      // 蛛网巨蛛：八足 + 头胸腹 + 骨螯，不要四足狼剪影
      pShadow(c, 48, 60, 24);
      pLine(c, 28, 36, 14, 22, 2.2, P_BARK);
      pLine(c, 34, 34, 20, 16, 2.2, P_BARK);
      pLine(c, 40, 50, 18, 62, 2.2, P_BARK);
      pLine(c, 46, 54, 28, 70, 2.2, P_BARK);
      pLine(c, 68, 36, 84, 20, 2.2, P_BARK);
      pLine(c, 62, 34, 80, 14, 2.2, P_BARK);
      pLine(c, 58, 50, 80, 62, 2.2, P_BARK);
      pLine(c, 52, 54, 72, 70, 2.2, P_BARK);
      pPoly(c, [[30, 48], [66, 46], [70, 34], [34, 36]], P_BARK);
      pCirc(c, 70, 36, 8, P_BARK);
      pPoly(c, [[74, 32], [86, 28], [80, 40]], P_BONE);
      pPoly(c, [[72, 38], [84, 42], [76, 46]], P_BONE);
  ...
  function renderCommandGrid(force) {
    ...
    if (activeTab === 'buildings' || activeTab === 'defense') {
      ...
      Object.keys(UNITS).forEach(function (key) {
        var u = UNITS[key];
        if ((u.producer === 'barracks' || u.producer === 'mtemple' || u.producer === 'tcamp') && sameFaction(u)) { definitions[key] = u; }
      });
    } else {
      // 载具页 = 工厂/召唤法阵出的单位
      definitions = {};
      Object.keys(UNITS).forEach(function (key) {
        var u = UNITS[key];
        if ((u.producer === 'factory' || u.producer === 'mcircle' || u.producer === 'tpen') && sameFaction(u)) { definitions[key] = u; }
      });
    }
